    ...
    public class EduSAutoTraderV1 : Strategy
    {
        ...
        protected override void OnBarUpdate()
        {
                ...
                && Position.MarketPosition != MarketPosition.Flat)
            {
                ...
                Print(string.Format(
                    "[AutoTrader] FIX#6 GUARDIA: Posición fantasma {0} {1} ctr detectada. Cerrando.",
                    Position.MarketPosition, qtyFantasma));
            ...
            if (_esperandoFillEntrada && !_entryFilled && _entryOrder != null)
            {
                ...
                bool hudInvalido   = !_hud._entradaValida;                           // señal pasó a histórica
                bool cooldownOk    = barrasActivas >= LimitCancelCooldownBarras;     // Fix #8: respetó el mínimo
                bool timeout       = barrasActivas >= LimitTimeoutBarras;             // límite duro

                bool cancelarPorHud = hudInvalido && cooldownOk;  // Fix #8: solo cancela si pasó el cooldown

                if (cancelarPorHud || timeout)
                {
                    string motivo;
                    if (timeout)
                        motivo = string.Format("timeout {0} barras sin fill", LimitTimeoutBarras);
                    else
                        motivo = string.Format("señal HUD inválida tras {0} barras (cooldown={1})",
                                               barrasActivas, LimitCancelCooldownBarras);
                    Print(string.Format("[AutoTrader#{0}] Cancelando Limit entrada — {1}.", _tradeId, motivo));
            ...
            {
                Print(string.Format("[AutoTrader#{0}] Cierre externo detectado. Cooldown {1} barras.", _tradeId, COOLDOWN_BARS));
            ...
            if (esNuevaSenal && !_enOperacion && !_esperandoFillEntrada)
            {
                ...
                {
                    Print(string.Format("[AutoTrader] Señal ignorada — cooldown post-cierre ({0}/{1} barras).",
                        CurrentBar - _barCierreExterno, COOLDOWN_BARS));
                    return;
                }
                EnviarEntrada();
            }
        ...
        private void EnviarEntrada()
        {
            ...
            string dirMargen = alcista
                ...
                : string.Format("{0:F2} + {1:F2} = {2:F2}", entrada, atrMargen, limitPrecio);
            Print(string.Format("[AutoTrader#{0}] Señal {1} {2}x → Limit={3:F2} ({4} | {5:F4}xATR) Stop={6:F2} T1={7:F2} T2={8:F2} (qt1={9} qt2={10}) CooldownCancel={11}b",
                _tradeId, alcista ? "LONG" : "SHORT", total,
        ...
        private void EnviarStopYTargets()
        {
            ...
            Print(string.Format("[AutoTrader#{0}] Fill confirmado. Stop={1:F2} T1={2:F2} T2={3:F2}",
                _tradeId, _stopInicial, _targetT1, _targetT2));
        ...
        private void MoverStopABreakeven()
        {
            ...
            Print(string.Format("[AutoTrader#{0}] T1 hit → Cancelando stop original ({1} ctr) y enviando nuevo Stop BE+{2}t = {3:F2} (qty={4})",
                _tradeId, _totalContratos, BEPlusTicks, nuevoStop, qtyRestante));
            ...
            OrderState orderState, DateTime time, ErrorCode error, string nativeError)
        {
            ...
            else
            {
                ...
                if (order.Name == "CloseAll" || order.Name == "SafeClose")
                {
                    ...
                    {
                        Print(string.Format("[AutoTrader#{0}] {1} completado → esperando sincronización flat.", _tradeId, order.Name));
                        CancelarOrdenesPendientes(cancelarStop: true, cancelarT1: true, cancelarT2: true);
                        _esperandoSincronizacionFlat = true;
                        _ticksEsperandoFlat = 0;
                    }
            ...
            {
                Print(string.Format("[AutoTrader#{0}] Limit entrada {1} por broker/sistema. Reseteando.",
                    _tradeId, orderState));
                ResetearEstado();
                return;
            }
            ...
            {
                Print(string.Format("[AutoTrader#{0}] Stop ejecutado. Cancelando T1/T2.", _tradeId));
            ...
            {
                Print(string.Format("[AutoTrader#{0}] T2 ejecutado. Cancelando stop BE residual.", _tradeId));
                CancelarOrdenesPendientes(cancelarStop: true);
        ...
        private void CancelarOrden(ref Order ord)
        {
            ...
            try
            {
                ...
                {
                    CancelOrder(ord);
                    Print("[AutoTrader] Cancelada orden: " + ord.Name);
                }
            }
            catch (Exception ex)
            {
                Print("[AutoTrader] Error cancelando orden: " + ex.Message);
            }
            ord = null;
        }
